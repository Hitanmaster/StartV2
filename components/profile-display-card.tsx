...
export default function ProfileDisplayCard({ profile, className }: ProfileDisplayCardProps) {
  const fallback = profile.avatarFallback || profile.name?.substring(0, 2).toUpperCase() || (
      ...
      <CardContent className="p-6 md:p-8 space-y-6">
        {profile.bio && (
          <div>
            ...
            <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap bg-card/50 p-4 rounded-md border border-border">
              {profile.bio}
            </p>
          </div>
        )}
        ...
        {profile.reason && (
          <div>
            ...
            <p className="text-muted-foreground leading-relaxed bg-secondary/30 p-4 rounded-md border border-secondary">
              {profile.reason}
            </p>
          </div>
        )}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {profile.interests.map((interest) => (
                <Badge key={interest} variant="outline" className="text-sm px-3 py-1.5 border-dashed">
                  {interest}
                </Badge>
              ))}
            </div>
